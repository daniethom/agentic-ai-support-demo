...
echo "🚀 Memory-Optimized Full AI Demo for CRC"
echo "========================================"

# Check current memory usage
echo "📊 Current Memory Usage:"
oc adm top nodes 2>/dev/null || echo "Node metrics will be available shortly"
echo ""

# Clean up any existing deployment first
echo "🧹 Cleaning up existing deployment..."
oc delete project agentic-ai-demo 2>/dev/null || echo "No existing project to clean"
sleep 10
...
oc new-project agentic-ai-demo

# Create memory-optimized deployment
echo "📝 Creating memory-optimized deployment..."
cat > k8s/crc-memory-optimized.yaml << 'EOF'
apiVersion: v1
kind: Namespace
metadata:
...
# Deploy with careful resource management
echo "🚀 Deploying memory-optimized version..."
oc apply -f k8s/crc-memory-optimized.yaml

echo ""
echo "📊 New Resource Allocation:"
echo "   Weaviate:     512-800MB RAM"
echo "   Ollama:       1-2GB RAM (reduced)"
echo "   API Service:  128-256MB RAM"
echo "   Streamlit:    256-512MB RAM"
echo "   ================================"
echo "   Total:        ~2-3.5GB RAM"
echo ""

# Wait for deployments in stages
echo "⏳ Deploying services in stages..."

# Stage 1: Weaviate first
echo "📦 Stage 1: Deploying Weaviate..."
oc rollout status deployment/weaviate -n agentic-ai-demo --timeout=180s

# Stage 2: API service (lightweight)
echo "📦 Stage 2: Deploying API service..."
oc rollout status deployment/support-api -n agentic-ai-demo --timeout=120s

# Stage 3: Streamlit app
echo "📦 Stage 3: Deploying web interface..."
oc rollout status deployment/ai-support-app -n agentic-ai-demo --timeout=120s

# Stage 4: Ollama last (most memory intensive)
echo "📦 Stage 4: Deploying Ollama (this may take longer)..."
if oc rollout status deployment/ollama -n agentic-ai-demo --timeout=300s; then
    ...
    # Try to pull a smaller model if memory allows
    echo "🤖 Attempting to pull model (may take several minutes)..."
    oc exec deployment/ollama -n agentic-ai-demo -- ollama pull tinyllama 2>/dev/null &
    echo "   Model download started in background (using tinyllama - smaller model)"
else
...
echo ""
echo "💾 Memory Usage:"
oc adm top pods -n agentic-ai-demo 2>/dev/null || echo "   Metrics will be available in a few minutes"
...
echo "   • Ollama uses smaller model (tinyllama) to fit memory"
echo "   • Vector search falls back to keyword matching if needed"
echo "   • All services have conservative memory limits"
echo "   • Total memory usage: ~2-3.5GB vs previous ~6GB"
echo ""
echo "🔍 Monitor status: oc get pods -n agentic-ai-demo"
echo "📊 Check memory: oc adm top pods -n agentic-ai-demo"