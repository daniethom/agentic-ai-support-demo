...
echo "🚀 Deploying FULL AI Demo (Optimized for Current CRC Resources)"
echo "=============================================================="

# Check current resource usage
echo "📊 Current CRC Resource Usage:"
oc adm top nodes 2>/dev/null || echo "Node metrics not available yet"
echo ""
...
# Create optimized deployment with resource limits that fit your available resources
echo "📝 Creating optimized full deployment..."
cat > k8s/crc-optimized-full.yaml << 'EOF'
apiVersion: v1
kind: Namespace
metadata:
...
# Deploy the optimized full version
echo "🚀 Deploying optimized full AI demo..."
oc apply -f k8s/crc-optimized-full.yaml

# Show resource allocation
echo ""
echo "📊 Resource Allocation for Full Demo:"
echo "   Weaviate:     1-1.5GB RAM, 250-500m CPU"
echo "   Ollama:       2-3GB RAM,   500m-1.5 CPU"  
echo "   API Service:  256-512MB,   200-500m CPU"
echo "   Streamlit:    512MB-1GB,   300-600m CPU"
echo "   =================================="
echo "   Total Est:    4-6GB RAM,   1.25-3.1 CPU"
echo ""
echo "💾 Your Available Resources:"
echo "   Available:    ~5.3GB RAM,  ~7+ CPU cores"
...
show_progress() {
    ...
    if oc rollout status deployment/$deployment -n $namespace --timeout=300s; then
        ...
        echo "⚠️  $deployment took longer than expected"
    fi
}

# Deploy services in order with progress tracking
show_progress "weaviate" "agentic-ai-demo"
show_progress "ollama" "agentic-ai-demo"  
show_progress "support-api" "agentic-ai-demo"
show_progress "ai-support-app" "agentic-ai-demo"
...
for i in {1..10}; do
    if oc exec deployment/ollama -n agentic-ai-demo -- ollama list >/dev/null 2>&1; then
        echo "   Ollama is ready, pulling llama3 model..."
        oc exec deployment/ollama -n agentic-ai-demo -- ollama pull llama3 &
        echo "   Model download initiated in background..."
        break
    else
...
if oc wait --for=condition=ready pod -l app=weaviate -n agentic-ai-demo --timeout=120s; then
    echo "   Weaviate is ready, you can now ingest knowledge data"
    echo "   Run: oc exec deployment/ai-support-app -n agentic-ai-demo -- python rag-setup/ingest_data.py"
else
    echo "   ⚠️  Weaviate not ready yet, knowledge base setup can be done later"
fi

# Get final status
echo ""
echo "📊 Final Deployment Status:"
oc get pods -n agentic-ai-demo -o wide

# Show resource usage
echo ""
echo "💾 Resource Usage:"
oc adm top pods -n agentic-ai-demo 2>/dev/null || echo "   Metrics will be available shortly..."

# Get routes
echo ""
echo "✅ Full AI Demo Deployment Complete!"
echo ""
...
echo "   ✅ Vector-based knowledge search"
echo "   ✅ Multi-tier AI agent simulation"
echo "   ✅ Real-time customer data lookup"
echo "   ✅ Automated troubleshooting guides"
echo "   ✅ AI-powered ticket routing"
echo "   ✅ Remote device management"
...
echo "   oc adm top pods -n agentic-ai-demo"
echo ""
